...
DENSE_REGULARIZER =layers.l2_regularizer(0.1)
LOSS_REGULARIZER = layers.l2_regularizer(0.1)
...
def cnn_model(image, labels, dropout_rate):
	...
	with tf.name_scope('conv2'):
		conv2 = conv2d(cdrop1, SECOND_FILTER)
	...
	return train_step, accuracy, logits
...
def train(_):
	...
	train_step, accuracy,_ = cnn_model(x, y, dropout_rate)

	saver = tf.train.Saver()
	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer())
		saver.restore(sess,"./regu_model/model.ckpt")
		for epoch in range(30):
			...
			for _ in range(int(len(train_x) / BATCH_SIZE)):
				...
				if not (_+1)%100:
					...
					saver.save(sess, "regu_model/model.ckpt")
...
def predict(_):
	...
	_,_,logits = cnn_model(x, y, dropout_rate)
	...
	submission.to_csv("results4.csv", index=False)
